...
const scanBssids = callable<[], {
    success: boolean;
    bssids?: BssidInfo[];
    error?: string
}>("scan_bssids");
const setBssid = callable<[bssid: string | null], {
    success: boolean;
    error?: string
}>("set_bssid");
const getCurrentBssid = callable<[], {
    success: boolean;
    bssid?: string | null;
    error?: string
}>("get_current_bssid");
...
export default definePlugin(() => ({
    ...
    titleView: <div className={staticClasses.Title}
                    style={{fontSize: '0.9em', padding: '5px', margin: '5px'}}>
        Network Manager
    </div>,
    content: <Content/>,
    icon: <FaWifi/>,
